...
- 可接入 OpenAI 兼容模型生成动态短句；默认使用 `gpt-5.6-luna` 的 Low 推理档位。
- LLM 默认每 60 秒自动生成一句；左键互动始终使用即时预设台词，不等待网络。
- 右键选择“打开控制台”，可以保存密钥、调整模型/超时/角色提示词、测试连接，并查看最近请求的成功或错误状态。
- 密钥只保存在 macOS 钥匙串；网络失败会立即使用本地台词，避免额外协议重试造成延迟。
...
项目使用 Swift Package Manager，无第三方运行时依赖。LLM 接口为可选功能；API Key 仅保存在 macOS 钥匙串中，不应写入源码或提交到仓库。
